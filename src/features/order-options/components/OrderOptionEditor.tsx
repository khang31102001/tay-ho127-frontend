...
import { MODIFIER_SELECTION_TYPE_OPTIONS, type ModifierSelectionType } from "@/features/modifier-groups/types/modifier-group.types";

import { useOrderOptionEditor } from "../hooks/useOrderOptionEditor";

type OrderOptionEditorProps = {
  id?: string;
};

export function OrderOptionEditor({ id }: OrderOptionEditorProps) {
  const {
    form,
    updateField,
    addOption,
    ...
    handleDelete,
    goToExplore,
  } = useOrderOptionEditor({ id });

  return (
    <DataEditor
      title={isEditMode ? "Sửa tùy chọn chung cho đơn hàng" : "Thêm tùy chọn chung cho đơn hàng"}
      backHref="/admin/settings/order-options"
      isLoading={isLoading}
      onSave={handleSave}
      onSaved={goToExplore}
